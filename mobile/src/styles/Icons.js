import React from 'react';
import {Icon} from '@ui-kitten/components';

export const HomeIcon = style => <Icon {...style} name="home-outline" />;
export const LoadingIcon = style => <Icon {...style} name="loader-outline" />;
export const LogInIcon = style => <Icon {...style} name="log-in-outline" />;
export const LogOutIcon = style => <Icon {...style} name="log-out-outline" />;
export const FilmOutlineIcon = style => <Icon {...style} name="film-outline" />;
export const RadioOutlineIcon = style => (
  <Icon {...style} name="radio-outline" />
);

export const RightArrowIcon = style => (
  <Icon {...style} name="arrow-forward-outline" />
);

export const AwardOutlineIcon = style => (
  <Icon {...style} name="award-outline" />
);
export const SearchIcon = style => <Icon {...style} name="search-outline" />;
export const PersonIcon = style => <Icon {...style} name="person-outline" />;
export const EditIcon = style => <Icon {...style} name="edit-outline" />;
export const HeartIcon = style => <Icon {...style} name="heart-outline" />;
export const CardIcon = style => <Icon {...style} name="credit-card" />;
export const TvIcon = style => <Icon {...style} name="tv" />;
export const BellIcon = style => <Icon {...style} name="bell" />;
export const LockIcon = style => <Icon {...style} name="lock" />;
export const FileTextIcon = style => (
  <Icon {...style} name="file-text-outline" />
);
export const CheckmarkCircleIcon = style => (
  <Icon {...style} name="checkmark-circle-outline" />
);
export const CloseCircleIcon = style => (
  <Icon {...style} name="close-circle-outline" />
);

export const BackIcon = style => (
  <Icon {...style} name="arrow-ios-back-outline" />
);

export const VolumeOffIcon = style => (
  <Icon {...style} name="volume-off-outline" />
);

export const VolumeOnIcon = style => (
  <Icon {...style} name="volume-up-outline" />
);

export const CameraIcon = style => <Icon {...style} name="camera" />;
export const GiftIcon = style => <Icon {...style} name="gift-outline" />;
